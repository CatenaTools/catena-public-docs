...
However, when set to the `PERMANENT_ONLY` policy, Catena will only report a player owns the corresponding Catena item
when they are the true, original owner; when a Steam App ID is shared with a player it will not be treated as owned by
that player.
...
attached, the non-admin lockout flag should be set or the value set by an admin will be overwritten on the next
ownership check.
